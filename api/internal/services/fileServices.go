...
func AddURLsToDownload(urlsRequest utils.DownloadRequest, status *utils.DownloadStatus) {
	...
	for _, url := range urlsRequest.URLs {
		status.UpdateStatus(url, utils.StatusResponse{Status: "pending", Message: "Descarga pendiente", TotalSize: 0, Progress: 0, DownloadID: url})
		go func(url string) {
			if err := utils.DownloadFile(url, destinationFolder+utils.ExtractFilename(url), status); err != nil {
				status.UpdateStatus(url, utils.StatusResponse{Status: "error", Message: err.Error()})
			} else {
